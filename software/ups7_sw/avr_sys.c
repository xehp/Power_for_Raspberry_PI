...
static void avr_relays_to_fail_state(void)
{
	RELAY_CHARGE_OFF();
	RELAY_SHUNT0_ON();
	RELAY_SHUNT1_ON();
...
static void avr_relays_enable(void)
{
	RELAY_CHARGE_ENABLE();
	RELAY_SHUNT0_ENABLE();
	RELAY_SHUNT1_ENABLE();
...
// Because if not we never get out of sleep (until reset).
void avr_wtd_reset_and_idle(void)
{
	...
	// This will make sure that the most important part of this SW is running.
	// NOTE It is important that no other SW module is calling wdt reset.
	// Those shall call avr_wtd_reset_and_idle instead of wdt reset.
	// This will not provide intended protection if someone else call wdt reset.
	if (power_wtd != 0)
	{
		wdt_reset();
		power_wtd--;
